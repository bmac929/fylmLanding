...
export const checkSubmissionLimit = (email, ip) => {
  ...
  const ipCount = ideas.filter(idea => 
    idea.ip_address === ip && 
    new Date(idea.created_at).getTime() > last24Hours
  ).length;
  
  return emailCount >= 2 || ipCount >= 2;
};
...
export const submitFilmIdea = async (formData, ipAddress) => {
  try {
    const ideas = loadFilmIdeas();
    
    // Check submission limit
    if (checkSubmissionLimit(formData.submitter_email, ipAddress)) {
...
export const getSubmissionCount = async (email, ipAddress) => {
  try {
    const ideas = loadFilmIdeas();
    const last24Hours = Date.now() - (24 * 60 * 60 * 1000);
